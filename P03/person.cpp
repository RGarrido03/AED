...
class person
{
  private:
    ...
    int d_phonenumber;
  public:
    person(const char *name = "",const char *address = "", const int phone = 0); // constructor
    ~person(void);                                                                // destructor
    void set_name(const char *name);
    void set_address(const char *address);
    void set_phone(const int phone);
    const char *get_name(void);
    const char *get_address(void);
    const int get_phone(void);
};
...
void person::set_phone(const int phone)
{
  d_phonenumber = phone;
}

person::person(const char *name,const char *address, const int phone)
{ // constructor
  if(debug != 0)
...
inline const int person::get_phone(void)
{
...
int main(void)
{
  person p = person("John Doe","no address",910000000); // create a new object and then copy it to p
